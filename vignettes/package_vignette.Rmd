...
First, we convert the BAM file(s) into coverage profiles and feature-extracted data frame. We will use the coverage profiles to visualize the mapping information. The feature-extracted data frame will be used in the model training and testing process. Note that both training data and testing data need to go through the conversion step. In our example, we only did the conversion for the test data because we are using the default training model later.
...
Next, we can visualize the coverage profile by using the `plot_cov` and the `plot_gc` functions.

* `plot_cov`: plots the coverage profile of the plant sample
* `plot_gc`: plots the percentage of A nucleotides and GC content for a sliding window of k-mer with the step as 1. We used the default setting of k = 75. 
